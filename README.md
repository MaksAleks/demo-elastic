...
- мы разделяем `content` каждого документа на отдельные слова (которые называем token-ы),
- сортируем токены
...
Анализ производится `анализатором (analyzer)`, который состоит из токенизатора `tokenizer` и нуля или более
фильтров `filters`
...
- Набрав в поиске "декоративная ваза" они не хотят получить в результате "декоративный горшок"  
  ...
  вещи выдавать в качестве результата не нужно.
...
Но пока что остановимся на типах данных.
...
У нас по прежнему остаются проблемы:
